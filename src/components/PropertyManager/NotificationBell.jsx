...
const NotificationBell = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('manager-notification-refresh', handleRefresh);
    
    return () => {
      clearInterval(interval);
      window.removeEventListener('manager-notification-refresh', handleRefresh);
    };
  }, []);

  const fetchUnreadCount = async () => {
    try {
      const data = await apiService.getManagerUnreadCount();
